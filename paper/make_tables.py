...
def macros(e1, e2, e3, e4, e5, e6, e7=None, e8=None, e9=None):
    ...
    if e7:
        h = e7["headline"]
        defs.update({
            "superEpisodes": h["episodes"],
            "superFailed": h["per_family_failed"],
            "superFailRate": pct(h["per_family_failure_rate"], 1),
            "superCurrentOk": h["current_decomposition_reproduced"],
        })
    if e8:
        ep = e8["episode"]
        defs.update({
            "exOrder": money(ep["order_cents"]),
            "exLoss": money(ep["infrastructure_cents"]),
            "exInstances": ep["n_instances"],
            "exShare": money(ep["share_each_cents"]),
            "exZeroed": ep["instances_given_nothing"],
            "exAgentOnly": money(ep["agent_only_top_cents"]),
    ...
    if e9:
        ...
        defs.update({
            ...
            "capBound": e9["search_cap"]["episodes_where_the_cap_bound"],
        })
...
def main():
    ...
    try:
        e7 = load("e7_superposition")
    except FileNotFoundError:
        e7 = None
    try:
        e8 = load("e8_worked_example")
    except FileNotFoundError:
        e8 = None
    try:
        e9 = load("e9_uncertainty")
    except FileNotFoundError:
        e9 = None
    try:
        e9 = load("e9_uncertainty")
    ...
    (ROOT / "paper" / "numbers.tex").write_text(macros(e1, e2, e3, e4, e5, e6, e7, e8, e9) + "\n")
